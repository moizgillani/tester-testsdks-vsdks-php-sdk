...
class WirelessNetworkPerformanceController extends BaseController
{
    ...
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/v1/intelligence/network-conditions')
            ->server(Server::M2M)
            ->auth('global')
    ...
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/v1/intelligence/wireless-coverage')
            ->server(Server::M2M)
            ->auth('global')
            ->parameters(HeaderParam::init('Content-Type', 'application/json'), BodyParam::init($body));
    ...
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/v1/intelligence/site-proximity/action/list')
            ->server(Server::M2M)
            ->auth('global')
            ->parameters(HeaderParam::init('Content-Type', 'application/json'), BodyParam::init($body));
    ...
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/v1/intelligence/device-experience/bulk/latest')
            ->server(Server::M2M)
            ->auth('global')
            ->parameters(HeaderParam::init('Content-Type', 'application/json'), BodyParam::init($body));
